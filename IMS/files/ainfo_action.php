...
if($_POST['btn_action'] == 'Add')
	...
	$profit = ($price_sold - $price_bought) ;
				...
				$query = "
		INSERT INTO airtime_info (customer_name,mobile_no,network_type, network_value,price_bought,price_sold,payment_mode,date1,status,remark,confirm_by,clientid,dataid,refcode,profit) 
	...
	{
		$query = "
		SELECT * FROM airtime_info WHERE product_id = :product_id
		";
		$statement = $connect->prepare($query);
		$statement->execute(
			array(
	...
	if($_POST['btn_action'] == 'delete')
	{
		...
		if(isset($result))
		{
			echo 'Airtime Info status change to ' . $status;
		}
	}
}
