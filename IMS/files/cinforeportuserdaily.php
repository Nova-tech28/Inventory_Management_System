...
$sql = "SELECT product_id,customername,mobileno,iucno,cablename,cableplan,costprice,sellprice,profit,date1,confirmby FROM cabletv WHERE  date1='$datekeep' and confirmby='$transname' and clientid='$clientid'  order by cablename ASC";
$result = $conn->query($sql);
//==============
$sql1="SELECT * FROM cabletv WHERE  date1='$datekeep' and confirmby='$transname' and clientid='$clientid' ";
...
$result5 = mysqli_query($conn,"SELECT SUM(sellprice) AS value_sum FROM cabletv WHERE date1='$datekeep' and confirmby='$transname' and clientid='$clientid' "); 
...
//----------------get total profit-----------------
$result7 = mysqli_query($conn,"SELECT SUM(profit) AS value_sum1 FROM cabletv WHERE date1='$datekeep' and confirmby='$transname' and clientid='$clientid' "); 
...
echo "<table class='table table-bordered table-striped'><thead><tr><th>S/NO</th><th>CUSTOMER NAME</th><th>MOBILE NO</th><th>IUC NO</th><th>CABLE NAME</th><th>CABLE PLAN</th><th>COST PRICE</th><th>SELL PRICE</th><th>PROFIT</th><th>CONFIRM BY</th></tr></thead>";
   ...
   //$c++ ;
         echo "<tr><td>" . ++$c. "</td><td>" . $row["customername"]. "</td><td>" . $row["mobileno"]. "</td><td>" . $row["iucno"]. "</td><td>" . $row["cablename"]. "</td><td>" . $row["cableplan"]. "</td><td>" . $row["costprice"]. "</td><td>" . $row["sellprice"]. "</td><td>" . $row["profit"]. "</td><td>" . $row["confirmby"]. "</td></tr>";
