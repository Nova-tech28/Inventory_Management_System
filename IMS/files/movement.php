        	...
        	<div class="modal-dialog">
        		<form method="post" id="user_form">
        			...
        			<div class="modal-header">
						...
						<h4 class="modal-title"><i class="fa fa-plus"></i> Staff Promotion</h4>
        			</div>
...
$query = mysqli_query($conn,"SELECT applicantname FROM recruitment where status = 'Active'and clientid = '$clientid' and clientid = '$clientid' order by applicantname asc");
 ...
 .'</option>';
                       ...
                       <div class="form-group">
                              ...
                              <div class="form-group">
                                ...
                                <select name="dept1" id="dept1" class="form-control" required>
                                    <?php 
									require "dbconfig.php";
									echo '<option value="">'.'--- Select New Department ---'.'</option>';
//$query=mysqli_query($con,"SELECT id,FirstName FROM persons");
...
$(document).ready(function(){
	...
	$('#add_button').click(function(){
		$('#user_form')[0].reset();
		$('.modal-title').html("<i class='fa fa-plus'></i> Staff Movement");
		$('#action').val("Add");
		$('#btn_action').val("Add");
	});
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('.modal-title').html("<i class='fa fa-pencil-square-o'></i> Edit Staff Movement");
				$('#rid').val(rid);
				$('#action').val('Edit');
				$('#btn_action').val('Edit');
