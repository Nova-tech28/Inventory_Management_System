...
$sql = "SELECT product_id,customer_rate,amount_sold,vendor_name,customer_name,card_name,card_amount,card_rate,rmb_value,profit,date1 FROM trading  WHERE  month1='$monthkeep' and year1='$yearkeep' and clientid='$clientid'  order by date1 DESC";
$result = $conn->query($sql);
//==============
$sql1="SELECT * FROM trading WHERE  month1='$monthkeep' and year1='$yearkeep' and clientid='$clientid' ";
...
$result5 = mysqli_query($conn,"SELECT SUM(amount_sold) AS value_sum FROM trading WHERE month1='$monthkeep' and year1='$yearkeep' and clientid='$clientid'"); 
...
//----------------get total profit-----------------
$result7 = mysqli_query($conn,"SELECT SUM(profit) AS value_sum1 FROM trading WHERE month1='$monthkeep' and year1='$yearkeep' and clientid='$clientid'"); 
...
echo "<table class='table table-bordered table-striped'><thead><tr><th>S/NO</th><th>CARD NAME</th><th>RATE</th><th>AMOUNT</th><th>VENDOR</th><th>RMB</th><th>CUSTOMER NAME</th><th>RATE</th><th>AMOUNT SOLD</th><th>PROFIT</th><th>SOLD DATE</th></tr></thead>";
	 ...
	 //$c++ ;
         echo "<tr><td>" . ++$c. "</td><td>" . $row["card_name"]. "</td><td>" . $row["card_rate"]. "</td><td>" . $row["card_amount"]. "</td><td>" . $row["vendor_name"]. "</td><td>"  . $row["rmb_value"]. "</td><td>" . $row["customer_name"]. "</td><td>" . $row["customer_rate"]. "</td><td>" . $row["amount_sold"]. "</td><td>" . $row["profit"]. "</td><td>" . $row["date1"]. "</td></tr>";
