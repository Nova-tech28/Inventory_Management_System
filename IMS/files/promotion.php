 ...
 .'</option>';
                       ...
                       <div class="form-group">
                         ...
                         <div class="form-group">
							...
							(Promotion)
							<input type="text" name="currentpost" id="currentpost" class="form-control" value="" required />
						</div>
                         <div class="form-group">
                                <label>Previous Salary</label>
                                <select name="psalary" id="psalary" class="form-control" required>
...
$(document).ready(function(){
	...
	$('#add_button').click(function(){
		$('#user_form')[0].reset();
		$('.modal-title').html("<i class='fa fa-plus'></i> Staff Promotion");
		$('#action').val("Add");
		$('#btn_action').val("Add");
	});
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#currentpost').val(data.currentpost);
				$('#salarymonth').val(data.salarymonth);
				$('#salaryannual').val(data.salaryannual);
				$('#approvedby').val(data.approvedby);
				$('.modal-title').html("<i class='fa fa-pencil-square-o'></i> Edit Staff Promotion");
				$('#rid').val(rid);
				$('#action').val('Edit');
				$('#btn_action').val('Edit');
