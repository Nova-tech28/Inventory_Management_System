...
foreach($result as $row)
{
	...
	{
		$payment_status = '<span class="label label-primary">Pre-Order</span>';
	}
	else
	{
	...
	$sub_array[] = '<a href="editorder.php?id='.$row["supplyid"].'" class="btn btn-info btn-xs">Confirm Order</a>';
...
{
	$statement = $connect->prepare("SELECT * FROM supply1 WHERE transstatus = 'Not Paid' AND status = 'Pre-order' AND clientid = '".$_SESSION['clientid']."'");
	$statement->execute();
