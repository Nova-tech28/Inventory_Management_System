...
{
	$query .= 'ORDER BY user_status ASC ';
}
...
foreach($result as $row)
{
	...
	$sub_array[] = '<button type="button" name="delete" id="'.$row["user_id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["user_status"].'">Deactivate</button>';
	$data[] = $sub_array;
}
...
{
	$statement = $connect->prepare("SELECT * FROM user_details WHERE clientid = '".$_SESSION['clientid']."'");
	$statement->execute();
