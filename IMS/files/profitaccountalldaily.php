...
$sql = "SELECT rid,itemname,itemquantity,amount,itemprice,product_profit,itembaseprice,confirmby FROM supply WHERE  supplydate='$datekeep' and clientid='$clientid'  order by itemname ASC";
$result = $conn->query($sql);
//==============
$sql1="SELECT * FROM supply WHERE  supplydate='$datekeep' and clientid='$clientid' ";
...
$result5 = mysqli_query($conn,"SELECT SUM(amount) AS value_sum FROM supply WHERE supplydate='$datekeep' and clientid='$clientid' "); 
...
//----------------get total profit-----------------
$result7 = mysqli_query($conn,"SELECT SUM(product_profit) AS value_sum1 FROM supply WHERE supplydate='$datekeep' and clientid='$clientid' "); 
...
echo "<table class='table table-bordered table-striped'><thead><tr><th>S/NO</th><th>ITEM NAME</th><th>COST PRICE</th><th>SELL PRICE</th><th>QUANTITY</th><th>AMOUNT</th><th>PROFIT</th><th>SOLD BY</th></tr></thead>";
	 ...
	 //$c++ ;
         echo "<tr><td>" . ++$c. "</td><td>" . $row["itemname"]. "</td><td>" . $row["itembaseprice"]. "</td><td>" . $row["itemprice"]. "</td><td>" . $row["itemquantity"]. "</td><td>"  . $row["amount"]. "</td><td>" . $row["product_profit"]. "</td><td>" . $row["confirmby"]. "</td></tr>";
