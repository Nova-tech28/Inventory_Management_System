...
if(isset($_POST["search"]["value"]))
{
	...
	$query .= 'OR possession LIKE "%'.$_POST["search"]["value"].'%") ';
}
...
foreach($result as $row)
{
	...
	$sub_array = array();
	$sub_array[] = $row['dataid'];
	$sub_array[] = $row['itemname'];
	$sub_array[] = $row['itemqty'];
	$sub_array[] = $row['possession'];
...
{
	$statement = $connect->prepare("SELECT * FROM casset WHERE clientid = '".$_SESSION['clientid']."'");
	$statement->execute();
